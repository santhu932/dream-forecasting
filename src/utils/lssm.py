...
LSSMContState = namedtuple('LSSMContState', ['mean', 'std', 'stoch', 'deter']) 
LSSMDiscState = namedtuple('RSSMDiscState', ['logit', 'stoch', 'deter'])

LSSMState = Union[LSSMDiscState, LSSMContState]

class LSSMUtils(object):
    '''utility functions for dealing with rssm states'''
    def __init__(self, lssm_type, info):
        self.lssm_type = lssm_type
        if lssm_type == 'continuous':
