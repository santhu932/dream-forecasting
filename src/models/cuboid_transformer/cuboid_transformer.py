...
class CuboidTransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.encode_latent = encode_latent
        ...
        if self.encode_latent:
            self.encode_z = nn.Conv2d(in_channels=T_in*base_units*downsample, 
                                      out_channels=T_out*base_units*downsample, 
    ...
    def forward(self, x, verbose=False):
        ...
        #print("After inital encoder", x.shape)
        x = self.enc_pos_embed(x)
        #print("After pos embd encoder", x.shape)
        if self.num_global_vectors > 0:
        ...
        #print("last layer", mem_l[-1].shape)
        #initial_z = self.get_initial_z(final_mem=mem_l[-1], T_out=T_out)
        
        z = mem_l[-1]
        if self.encode_latent:
            _, _, hidden_H, hidden_W, hidden_C = z.shape
            initial_z = self.encode_z(z.permute(0, 1, 4, 2, 3).reshape(B, T_in * hidden_C, hidden_H, hidden_W))
            initial_z = initial_z.reshape(B, T_out, hidden_C, hidden_H, hidden_W).permute(0, 1, 3, 4, 2)
